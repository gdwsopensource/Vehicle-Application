...
public interface BehaviorPredictionRepository extends JpaRepository<BehaviorPrediction, Integer> {
	 ...
	 * @return
	 */
	@Query(value = "select co.id as id,co.cross_id as cross_id,ci.cross_name as cross_name,co.plate_no as plate_no,co.cross_date as cross_date ,co.alert_type as alert_type from car_overview_cross co join cross_info ci on co.cross_id=ci.cross_id where co.plate_no=?1", nativeQuery = true)
